...
There is no native skill-to-skill dependency management. A dependent skill must check bridge availability itself before assuming the bridge is usable.
...
B. Bridge running, bot offline
   ...
   (GET /config, /memory, /state still work in this state — useful for diagnosis)
...
> **Localhost proxy note**: if `curl` to `127.0.0.1` fails with
> `upstream connect failed` / `os error 10061`, a proxy is intercepting the request.
